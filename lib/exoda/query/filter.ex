defmodule Exoda.Query.Filter do
  ...
  def add_filter(query_string_params, %Ecto.Query{wheres: wheres, sources: sources}, params) do
    Logger.info("Query wheres: #{inspect(wheres)}")
    filter = do_add_filter(wheres, sources, params, "")
    Map.put(query_string_params, "$filter", filter)
  end

  @spec do_add_filter([Ecto.Query.QueryExpr.t], sources, params, String.t) :: String.t
  defp do_add_filter([], _, _, acc), do: acc
  defp do_add_filter([%{expr: expr, op: op} | tail], sources, params, acc) do
    new_filter_expr = build_filter_expr(expr, sources, params)
    updated_query = 
      case acc do
        "" -> "(#{new_filter_expr})"
        query -> "#{query} #{convert_op(op)} (#{new_filter_expr})"
      end
    do_add_filter(tail, sources, params, updated_query)
  end
  
  @spec build_filter_expr(any, sources, params) :: String.t
  # is_nil(expr) -> expr eq null
  defp build_filter_expr({:is_nil, [], [expr]}, sources, params) do 
    "(#{build_filter_expr(expr, sources, params)}) eq null"
  end

  # not(expr) -> not (expr)
  defp build_filter_expr({:not, [], [expr]}, sources, params) do 
    "not (#{build_filter_expr(expr, sources, params)})"
  end
  
  # Aggregation functions are not supported
  defp build_filter_expr({op, [], _}, _, _) when op in @aggregation_functions do
    throw "min/1, max/1, sum/1 and avg/1 functions are not supported by Exoda adapter"
  end

  # Datetime functions are not supported
  defp build_filter_expr({op, [], _}, _, _) when op in @date_functions do
  ...
  defp build_filter_expr({:^, _, [index]}, _, params) do
    Enum.at(params, index)
  end

  # Get field from source schema
  defp build_filter_expr({{:., [], [{:&, [], [0]}, field_name]}, _, []}, _, _) do
    field_name
  end

  # Get field from joined schemas
  defp build_filter_expr({{:., [], [{:&, [], [source_idx]}, field_name]}, _, []}, sources, _) do
    get_assoc_field(sources, source_idx, field_name)
  end

  # Type casting is not supported
  defp build_filter_expr(%Ecto.Query.Tagged{type: _}, _, _) do
  ...
  defp build_filter_expr({:in, _, _}, _, _) do
    throw "Inclusion operator in/2 is not supported by Exoda adapter"
  end

  # Boolean operators
  defp build_filter_expr({op, _, [left, right]}, sources, params) when op in [:and, :or] do
    "(#{build_filter_expr(left, sources, params)}) #{convert_op(op)} (#{build_filter_expr(right, sources, params)})"
  end
  ...
  defp build_filter_expr({:like, _, [expr, value]}, sources, params) do
    source = build_filter_expr(expr, sources, params)
    ...
    cond do
      ...
      :otherwise ->
        build_filter_expr({:==, [], [expr, value]}, sources, params)
    end
  end

  # Build filter from fragment expression
  defp build_filter_expr({:fragment, _, parts}, sources, params) do
    build_fragment(parts, sources, params)
  end

  # Other binary expressoins, like comparison operators etc
  defp build_filter_expr({op, _, [left, right]}, sources, params) do
    "#{build_filter_expr(left, sources, params)} #{convert_op(op)} #{build_filter_expr(right, sources, params)}"
  end
  ...
  # 'String' -> 'String'
  # 12 -> 12
  defp build_filter_expr(string, _, _) when is_binary(string), do: "'#{string}'"
  defp build_filter_expr(literal, _, _), do: literal
  ...
  defp build_fragment(parts, sources, params) do
    Enum.reduce(parts, "", fn 
      {:raw, str}, acc -> "#{acc}#{str}"
      {:expr, expr}, acc -> "#{acc}#{build_filter_expr(expr, sources, params)}"
    end)
  end
end
