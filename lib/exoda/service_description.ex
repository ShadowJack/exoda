defmodule Exoda.ServiceDescription do
  use Agent
  require Logger
  import SweetXml

  @moduledoc """
  Agent that stores configuration and description of the remote
  OData service
  """
  ...
  @doc """
  Provide Supervisor.Spec.worker description so that external supervisor
  would know how to run the Exoda.ServiceDescription process.
  ...
  def child_spec(_repo, opts) do
    Supervisor.Spec.worker(Exoda.ServiceDescription, [opts])
  end

  @doc """
  Read configuration of the adapter
  """
  ...
  def init(opts) do
    ...
    with {:ok, repo} <- Keyword.fetch(opts, :repo),
         ...
         {:ok, %{meta_url: meta_url, odata_version: odata_version}} <-
           fetch_service_info(service_url),
      ...
      nil -> raise "Missing configuration of Ecto repo"
  ...
  defp fetch_service_info(service_url) do
    response = @client.get!(service_url, [{"Accept", "application/json"}], follow_redirect: true)

    if response.status_code != 200 do
      {:error,
       "Ecto configuration error: service url #{service_url} is not accessible. Status code: #{
  ...
  defp extract_odata_verison(%HTTPoison.Response{headers: headers, request_url: service_url}) do
    ...
    case version_header do
      ...
      {_name, version} ->
        {:error,
         "OData service #{service_url} has unsupported version: #{version}. Only OData v4 is supported by Exoda adapter."}

      nil ->
        {:error,
         "Version info is not provided by OData service #{service_url}. Only OData v4 is supported by Exoda adapter."}
    end
  end
  ...
  defp fetch_metadata(meta_url) do
    response = @client.get!(meta_url, [{"Accept", "application/xml"}], follow_redirect: true)
  ...
  defp parse_metadata_response(%HTTPoison.Response{
         status_code: status_code,
         request_url: request_url,
         body: body
       }) do
    {:error,
     "Error while getting metadata of OData service at #{request_url}. Status code: #{status_code}. Body: #{
       body
     }"}
  end
end
